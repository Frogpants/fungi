...
void main() {
    ...
    vec3 color = texture(modelTexture, pixelUV).rgb;

    float dist = length(uv);
    float edge = smoothstep(1.6, 2.0, dist);
    vec3 glow = vec3(0.08, 0.05, 0.03) * edge;
    color = mix(color, glow, edge);

    color += texture(modelTexture, pixelUV + vec2(1.0 / resolution.x, 0.0)).rgb * 0.15;
    color -= texture(modelTexture, pixelUV - vec2(1.0 / resolution.x, 0.0)).rgb * 0.1;
